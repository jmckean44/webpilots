...
class Trig {
	...
	calculatePosTop(target, el, options) {
		...
		return options.negativeOffset > 0 ? -(el - (halfHeight + options.negativeOffset)) : -(el - (halfHeight - options.offset));
	}
...
document.addEventListener('astro:page-load', trig.trigInit);
